...
export const exportUserData = async () => {
  try {
    ...
    const settings = {
      theme: localStorage.getItem('peergenius-theme') || 'light',
      language: localStorage.getItem('peergenius-language') || 'en',
      notifications: JSON.parse(localStorage.getItem('peergenius-notifications') || '{}'),
      preferences: JSON.parse(localStorage.getItem('peergenius-preferences') || '{}')
    };
    ...
    a.href = url;
    a.download = `peergenius-data-${new Date().toISOString().split('T')[0]}.json`;
...
export const getUserSettings = () => {
  return {
    theme: localStorage.getItem('peergenius-theme') || 'light',
    language: localStorage.getItem('peergenius-language') || 'en',
    notifications: JSON.parse(localStorage.getItem('peergenius-notifications') || JSON.stringify({
      email: true,
      push: false,
      threadUpdates: true,
      aiResponses: true,
    ...
    preferences: JSON.parse(localStorage.getItem('peergenius-preferences') || JSON.stringify({
      autoSave: true,
      compactMode: false
    }))
  };
...
export const saveUserSettings = (settings) => {
  try {
    localStorage.setItem('peergenius-theme', settings.theme);
    localStorage.setItem('peergenius-language', settings.language);
    localStorage.setItem('peergenius-notifications', JSON.stringify(settings.notifications));
    localStorage.setItem('peergenius-preferences', JSON.stringify(settings.preferences));
